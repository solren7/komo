...
- operator surface: `komo policy list` / `komo policy check` (`cli/policy.rs`, pure config parsing — no db/gateway involved) and a `policy:` section in `komo doctor`
