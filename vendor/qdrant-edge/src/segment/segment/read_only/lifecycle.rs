...
use crate::common::storage_version::{StorageVersion, VERSION_FILE};
...
use crate::segment::index::read_only::{ReadOnlyVectorIndexOpenArgs, VectorIndexReadEnum};
use crate::segment::index::struct_payload_index::read_only::ReadOnlyStructPayloadIndex;
use crate::segment::payload_storage::read_only::ReadOnlyPayloadStorage;
use crate::segment::segment::{SEGMENT_STATE_FILE, SegmentVersion};
...
use crate::segment::vector_storage::quantized::quantized_vectors::ReadOnlyQuantizedVectors;
...
impl<S: UniversalReadExt + 'static> ReadOnlyVectorData<S> {
    ...
    /// `raw_fs` is the segment's raw backend, retained by the HNSW index for
    /// a deferred graph load (the caching `fs` only lives for this open).
    #[allow(clippy::too_many_arguments)]
    ...
    pub(super) fn open_sparse(
        ...
        // The sparse open reads lazily, so a profile that never scores this
        // vector just parks the index data cold — no deferral needed (see
        // `LoadProfile::vector_index_placement`).
        let index_populate =
            load_profile.and_then(|profile| profile.vector_index_placement(vector_name));
