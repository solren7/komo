...
- 操作面:`shion dream`(预览)每行加 `queries=N`;`cli/memory.rs::line` 在 recall_count 后
  追加同款标注,`shion memory report` 不动。

迁移:`ensure_columns` 原地加列,memory.db 是 durable 数据,**用户不删库**。

### G2: candidate 堆清不动——单 id 操作 + gateway 锁把 CLI triage 堵死

`shion memory promote/reject/pin` 一次一个 id,且都套着 `refuse_if_gateway_running`——
gateway 常驻(常态)时唯一的 triage 通道是去 chat 里让 agent 调 memory tool,逐条对话式操作。
`shion memory report` 能看见堆,却清不了。
...
- `promote` / `reject` 接受变参:`shion memory promote <id>...`,逐条报结果。
- 新增 `shion memory triage`:交互式过 candidate 堆(排序与 report 一致,最老优先——
...
- aux 合约(输入:用户消息 + 编号候选列表,每条带 id/kind/confidence/内容):
  ...
  `line` 可缺省(缺省则用原文)。shion 侧校验:id 必须 ∈ 候选集(伪造即丢弃),
  解析失败 / 超时 / keep 为空 → **回落到按分数 top-5 直注**,与现有 recall 失败契约一致
  (非致命、warn 日志)。
- **防注入边界不破**:aux 输出不作为自由文本进 prompt——选择结果仍走
  `render_recalled_memory_block` 渲染(source 标签、untrusted caveat、`<!-- shion:memory:recall -->`
...
| 03 | memory-triage-cli | `shion memory triage` 交互式清堆(依赖 02) | 小 |
...
- **aux 的「解释」字段**:见 G3 末尾。
- **动 active/user-saved 记忆的自动治理**:dreaming 只动 candidate 的原则不变,active 的退役仍归操作员(`shion memory report`)。
