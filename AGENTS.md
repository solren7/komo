...
- `Skill` carries governance frontmatter next to identity: `protected` (operator-edit-only — the reviewer writes **no** candidate proposal, so a "just promote it" nudge can never overwrite the operator's version), `disabled` (kept on disk + inspectable, hidden from the model's catalog; `skill view` answers with its state, not its instructions), `source` (`user` | `reviewer` provenance). `valid_skill_name` is the path-segment floor that keeps an LLM-suggested name inside the skills tree
