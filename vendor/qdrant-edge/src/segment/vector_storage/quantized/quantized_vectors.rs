...
pub(in crate::segment::vector_storage::quantized) type ReadFile = MmapFile;
...
pub(in crate::segment::vector_storage::quantized) const READ_FS: MmapFs = MmapFs;

pub(in crate::segment::vector_storage::quantized) use self::config::QuantizedStorageKind;
pub use self::config::{
...
use crate::segment::vector_storage::quantized::quantized_scorer_builder::{
...
impl QuantizedVectors {
    ...
    pub(in crate::segment::vector_storage::quantized) fn get_config_path(path: &Path) -> PathBuf {
        path.join(QUANTIZED_CONFIG_PATH)
    }

    pub(in crate::segment::vector_storage::quantized) fn get_data_path(
        path: &Path,
    ...
    pub(in crate::segment::vector_storage::quantized) fn get_meta_path(path: &Path) -> PathBuf {
        path.join(QUANTIZED_META_PATH)
    }

    pub(in crate::segment::vector_storage::quantized) fn get_offsets_path(
        path: &Path,
        storage_type: QuantizedVectorsStorageType,
    ) -> PathBuf {
        match storage_type {
    ...
    pub(in crate::segment::vector_storage::quantized) fn memory_placement(
    ...
    pub(in crate::segment::vector_storage::quantized) fn is_ram(
        requested: Option<Memory>,
        on_disk_vector_storage: bool,
    ) -> bool {
    ...
    pub(in crate::segment::vector_storage::quantized) fn convert_binary_encoding(
    ...
    pub(in crate::segment::vector_storage::quantized) fn convert_tq_bits(bits: TurboQuantBitSize) -> TQBits {
        match bits {
    ...
    pub(in crate::segment::vector_storage::quantized) fn tq_bits_default_rescoring(bits: TQBits) -> bool {
        match bits {
    ...
    pub(in crate::segment::vector_storage::quantized) fn construct_vector_parameters(
    ...
    pub(in crate::segment::vector_storage::quantized) fn get_bucket_size(
    ...
    pub(in crate::segment::vector_storage::quantized) fn quantized_vector_size(
        quantization_config: &QuantizationConfig,
        ...
        match quantization_config {
            QuantizationConfig::Scalar(_) => {
                crate::quantization::encoded_vectors_u8::get_quantized_vector_size(vector_parameters)
            }
            QuantizationConfig::Product(ProductQuantization { product }) => {
                let bucket_size = Self::get_bucket_size(product.compression);
                crate::quantization::encoded_vectors_pq::get_quantized_vector_size(
                    vector_parameters,
                    bucket_size,
                )
            }
            QuantizationConfig::Binary(BinaryQuantization { binary }) => {
                ...
                if is_multi {
                    crate::quantization::encoded_vectors_binary::get_quantized_vector_size_from_params::<u8>(
                        vector_parameters.dim,
                        encoding,
                    )
                } else {
                    crate::quantization::encoded_vectors_binary::get_quantized_vector_size_from_params::<
                        u128,
            ...
            QuantizationConfig::Turbo(TurboQuantization { turbo }) => {
                ...
                crate::quantization::encoded_vectors_tq::get_quantized_vector_size(
                    vector_parameters,
                    bits,
                    crate::quantization::turboquant::TQMode::Plus,
                )
            }
        }
    }
