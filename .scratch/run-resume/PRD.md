...
| `shion run list/inspect/prune` | `cli/inspect.rs`,经 gateway `GET /api/runs*` 路由 | ✅ |
...
- schema 变更:`RunRecord` 加列 = 删 `shion.db` 重建(AGENTS.md 既有约定,disposable db 不做 ALTER 迁移)。
...
- **gateway 不在**:CLI 直接开 db + `cli/wiring.rs` 构建 runtime 本地跑(与 `shion chat` 同构)。
- CLI:`shion run resume [<id>]`——缺省取最新一条 recoverable(`run list` 顺序);找不到时提示"没有可恢复的 run"。
...
| 02 | resume-core | priming 组装(纯函数,可测)+ 校验 + 本地直连路径 + `shion run resume` | 中 |
