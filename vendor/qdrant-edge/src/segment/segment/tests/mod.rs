...
fn test_snapshot_streamable_without_files_wrapper() {
    ...
    crate::segment::utils::fs::move_all(&files_path, &segment_path).unwrap();
...
fn test_retrieve_raw_sparse_bytes() {
    ...
    use crate::segment::vector_storage::sparse::StoredSparseVector;
...
fn test_append_only_delete_is_tombstone_only_on_non_appendable() {
    ...
    use crate::segment::index::PayloadIndexRead as _;
...
fn test_upsert_raw_malformed_blob_rejected() {
    ...
    assert!(
        matches!(
            result,
            Err(crate::segment::common::operation_error::OperationError::MalformedVectorBlob { .. })
        ),
