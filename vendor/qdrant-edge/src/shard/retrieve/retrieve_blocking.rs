...
use crate::segment::data_types::segment_record::SegmentRecordRaw;
...
/// Byte-blob analogue of [`retrieve_blocking`]: returns vectors as
...
pub fn retrieve_raw_blocking(
    ...
    let segments: Vec<_> = {
        let Some(holder_guard) = segments.try_read_for(timeout) else {
            return Err(OperationError::timeout(timeout, "retrieve_raw"));
        };
        holder_guard
            .non_appendable_then_appendable_segments()
            .map(|segment| segment.get_read_arc())
