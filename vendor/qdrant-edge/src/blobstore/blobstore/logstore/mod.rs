...
use crate::blobstore::config::{LogstoreConfig, StorageConfig};
use crate::blobstore::error::BlobstoreError;
use crate::blobstore::tracker::append_only::AppendOnlyTracker;
use crate::blobstore::tracker::{PointOffset, ValuePointer};
    ...
    S: UniversalAppend + 'static,
{
    ...
    pub(super) fn get_pointer(&self, point_offset: PointOffset) -> Option<ValuePointer> {
        self.tracker
            .read()
            .get::<crate::common::generic_consts::Random>(point_offset)
            .ok()
            .flatten()
    }
