...
fn test_storage_files(#[values(Mode::Mutable, Mode::AppendOnly)] mode: Mode) {
    ...
    let expected_files: &[&str] = match mode {
        ...
        Mode::AppendOnly => &["log_tracker.dat", "log_page_0.dat", "config.json"],
    };
    assert_eq!(
        files.len(),
        expected_files.len(),
...
fn test_different_block_sizes(
    ...
    use crate::blobstore::fixtures::minimal_payload;
...
fn test_live_reload_across_pages() {
    use crate::blobstore::config::DEFAULT_BLOCK_SIZE_BYTES;
    use crate::blobstore::fixtures::minimal_payload;
...
fn test_preopen_schedules_files_for_open(#[values(Mode::Mutable, Mode::AppendOnly)] mode: Mode) {
    ...
    let backend_read_files: &[&str] = match mode {
        ...
        Mode::AppendOnly => &["config.json", "log_page_0.dat"],
    };
