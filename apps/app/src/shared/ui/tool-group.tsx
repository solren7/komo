...
//   bunx shadcn@latest add @assistant-ui/tool-group
//
// Two deliberate departures from upstream, both re-apply on a re-add:
//   1. `ToolGroupTrigger` takes an optional `label` and trailing `children`.
//      Upstream hardcodes the English `N tool calls`; komo's collapsed line is
//      Chinese and also names the tools that ran (see features/chat/ToolCalls).
//   2. The deprecated default-export `ToolGroup` wrapper is gone. It exists for
//      the legacy `components.ToolGroup` prop; komo groups with
//      `MessagePrimitive.GroupedParts` + `groupPartByType`, which is what
//      upstream's own deprecation note points at, and composes Root/Trigger/
//      Content directly.
...
function ToolGroupTrigger({
  ...
  count: number;
  active?: boolean;
  /** Overrides the default `N tool calls`. The shimmer traces whatever this is,
   *  so it must be plain text, not markup. */
  label?: string;
  /** Rendered between the label and the chevron — extra detail on the collapsed
   *  line that should not shimmer (komo puts the tool names there). */
  children?: ReactNode;
}) {
