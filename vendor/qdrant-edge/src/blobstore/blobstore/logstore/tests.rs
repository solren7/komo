...
use crate::common::universal_io::{MmapFile, MmapFs, Populate};
use fs_err as fs;
use tempfile::TempDir;

use crate::blobstore::blobstore::reader::CONFIG_FILENAME;
use crate::blobstore::config::{Compression, DEFAULT_PAGE_SIZE_BYTES, LogstoreConfig, Mode, StorageConfig};
use crate::blobstore::error::BlobstoreError;
use crate::blobstore::fixtures::{Payload, default_config, empty_storage_append_only, random_payload};
use crate::blobstore::tracker::ValuePointer;
use crate::blobstore::{Blobstore, BlobstoreReader};
...
fn test_open_or_create_keeps_mode_on_disk() {
    ...
    let config = StorageConfig::AppendOnly(LogstoreConfig::DEFAULT);
    let storage =
        Blobstore::<Payload>::open_or_create(MmapFs, path.clone(), config, Populate::No).unwrap();
    storage.as_logstore();
    drop(storage);

    // Opening again ignores the create config, the mode comes from the persisted config
    let storage = Blobstore::<Payload>::open_or_create(
        MmapFs,
        path,
        default_config(Mode::Mutable),
        Populate::No,
    )
    .unwrap();
    storage.as_logstore();
...
fn small_page_storage(dir: &TempDir, page_size_bytes: usize) -> Blobstore<Vec<u8>> {
    ...
    Blobstore::new(MmapFs, dir.path().to_path_buf(), config).unwrap()
}
